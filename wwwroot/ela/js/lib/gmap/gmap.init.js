    ...
    map.loadFromFusionTables({
        ...
        events: {
            click: function(point){
                ...
                infoWindow.setPosition(point.latNewDepot);
...
$(document).ready(function(){
    ...
    var NewDepot = -77.02024422636042;
    ...
    var b = [];
    for(var i in bounds){
        latNewDepot = new google.maps.LatNewDepot(bounds[i][0], bounds[i][1]);
        b.push(latNewDepot);
    }
    for(var i in paths){
        latNewDepot = new google.maps.LatNewDepot(paths[i][0], paths[i][1]);
        b.push(latNewDepot);
    }
    map.fitLatNewDepotBounds(b);
});
            ...
            if (status == google.maps.ElevationStatus.OK) {
                for (var i in result){
                    map.addMarker({
                        ...
                        NewDepot: result[i].location.NewDepot(),
