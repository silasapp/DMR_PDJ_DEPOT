$(function () {
    ...
    $("#FormCreateRole").on('submit', function (event) {
        ...
        if ($("#txtRoleName").val() === "") {
            ...
            if (msg === true) {
                $.getJSON("/UserRoles/CreateRoles", { "RoleName": $("#txtRoleName").val() }, function (response) {
        ...
        columns: [
            ...
            { data: "updatedAt" },
            {
                "render": function (data, type, row) {
                    return "<button class=\"btn btn-sm btn-primary role" + row["roleID"] + "\" id=\"" + row['roleName'] + "\" onclick=\"getRole(" + row["roleID"] + ")\"> <i class=\"fa fa-edit\"> </i> Edit </button> <button class=\"btn btn-sm btn-danger\" target=\"\" onclick=\"DeleteRole(" + row["roleID"] + ")\"> <i class=\"fa fa-trash-o\"> </i> Delete </button>";
                }
            }
        ],
