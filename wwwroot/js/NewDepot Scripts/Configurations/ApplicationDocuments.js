$(function () {
    ...
    $("#FormCreateAppDoc2").on('submit', function (event) {
        ...
        if ($("#txtAppDocName2").val() === "") {
            ...
            if (msg === true) {
                $.getJSON("/ApplicationDocuments/CreateAppDoc", {
                    "AppDocElpsID": $("#txtAppDocName2").val(),
                    "AppDocName": $("#txtAppDocName2 option:selected").text(),
                    "PhaseName": $("#txtPhaseName option:selected").text(),
                    "AppDocType": "Facility"
                }, function (response) {
                    if ($.trim(response) === "AppDoc Created") {
        ...
        columns: [
            ...
            { data: "updatedAt" },
            {
                "render": function (data, type, row) {
                    return "<button class=\"btn btn-xs btn-primary doc" + row["appDocID"] + "\" id=\"" + row['docName'] +"|"+ row['phaseName'] + "\" onclick=\"getAppDoc(" + row["appDocID"] + ")\"> <i class=\"fa fa-edit\"> </i> Edit </button> <button class=\"btn btn-xs btn-danger\" target=\"\" onclick=\"DeleteAppDoc(" + row["appDocID"] + ")\"> <i class=\"fa fa-trash-o\"> </i> Delete </button>";
    ...
    $("#txtAppDocName1").ready(function () {
        ...
        $.getJSON("/Helpers/GetElpsDocumentsTypess",
            function (datas) {
    ...
    $("#txtAppDocName2").ready(function () {
        ...
        $.getJSON("/Helpers/GetElpsFacDocumentsTypess",
            function (datas) {
                ...
                $.getJSON("/ApplicationDocuments/EditAppDoc",
                    {
                        ...
                        "PhaseName": $("#txtEditPhaseName option:selected").val(),
...
function getAppDoc(id) {
    ...
    var app = $(".doc" + id).attr("id").split("|");
  

    $("#txtEditAppDocName").val(app[0]);
    $("#txtEditPhaseNameVal").val(app[1]);
    $("#txtEditPhaseNameVal").text(app[1]);
